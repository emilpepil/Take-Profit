...
interface IEoaTakeProfitPoolV2 {
...
/// @notice Multi-rule EOA take-profit executor for Monad Testnet.
/// @dev Assets remain in each user's wallet until an individual target is reached.
contract EoaTakeProfitExecutorV2 is ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public constant BPS_DENOMINATOR = 10_000;
    uint256 public constant MAX_POLICIES_PER_OWNER = 12;

    IEoaTakeProfitPoolV2 public immutable pool;
    ...
    constructor(IERC20 asset_, IERC20 stable_, IEoaTakeProfitPoolV2 pool_, address keeper_) {
    ...
    /// @notice Creates one additional take-profit level for the caller.
    /// @dev The aggregate active amount must be backed by the wallet balance and ERC-20 allowance.
    function createPolicy(uint256 amount, uint256 targetPriceE18, uint16 maxSlippageBps) external returns (uint256 policyId) {
        ...
        if (policyCount[msg.sender] >= MAX_POLICIES_PER_OWNER) revert TooManyPolicies();
    ...
    /// @notice Stops one rule. It never transfers caller tokens.
    ...
    /// @notice Keeper-only execution of one rule after that rule's target is met.
